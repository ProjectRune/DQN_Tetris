...
class dqn_agent(Agent):  # RL-glue Process
    ...
    def agent_init(self, taskSpec):
        ...
        # self.DQN = DQN_class(enable_controller=[0, 1, 3, 4, 5, 8, 9])  # enduro
        # self.DQN = DQN_class(enable_controller=[0, 2, 3, 4, 5])  # pacman
        # self.DQN = DQN_class(enable_controller=[0, 1, 3, 4, 5, 11, 12, 13])  # phoenix
        # self.DQN = DQN_class(enable_controller=[0, 1, 3, 4, 11, 12])  # SpaceInvaders
        # self.DQN = DQN_class(enable_controller=[0, 1, 3, 4])  # tetris

    def agent_start(self, observation):

	print(str(len(observation.intArray)))
        # Preprocess
        tmp = np.bitwise_and(np.asarray(observation.intArray[128:]).reshape([264, 192]), 0b0001111)  # Get Intensity from the observation
    ...
    def agent_step(self, reward, observation):
        ...
        with open('observation.dat', 'wb') as f:
    ...
    def agent_message(self, inMessage):
        ...
        if inMessage.startswith("save model"):
            with open('dqn_model_' + str(self.episode) + '.dat', 'w') as f:
